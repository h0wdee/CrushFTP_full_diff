...
import com.crushftp.tunnel.Downloader2;
import com.crushftp.tunnel.Uploader2;
...
implements DropTargetListener {
    ...
    public static Applet2 thisObj = null;
    ...
    public void init() {
        ...
        new Thread(new Runnable(){
            ...
            @Override
            public void run() {
                Thread.currentThread().setName("Applet log memory protector");
                while (true) {
                    try {
                        Thread.sleep(1000L);
                    }
            ...
            try {
                ...
                if (p != null) {
                    ...
                    if (p.getProperty("COMMAND").equalsIgnoreCase("AUTH")) {
                        ...
                        this.doAction(p);
                    } else if (p.getProperty("COMMAND").equalsIgnoreCase("LOG")) {
                        this.doLog(p);
                    } else if (p.getProperty("COMMAND").equalsIgnoreCase("ASK_RESPONSE")) {
                        this.doAskResponse(p);
        ...
        new Thread(new Runnable(){
            ...
            public void run() {
                try {
                    ...
                    Applet2.this.commands.addElement(p);
                    while (!p.containsKey("result")) {
                        Thread.sleep(100L);
                    }
                    Applet2.this.resultsAsync.put(myId, p.getProperty("result_data", ""));
                }
                catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public void doList(Properties p) {
        try {
            ...
            if (p.getProperty("PATH").equals("/") && Common.machine_is_windows()) {
            ...
            while (files != null && x < files.length) {
                ...
                String can_path = files[x].getCanonicalPath().replace('\\', '/');
                if (Common.machine_is_windows() && p.getProperty("PATH").equals("/")) {
                    name = String.valueOf(files[x].getPath().substring(0, 2)) + "/";
                    can_path = String.valueOf(files[x].getPath().substring(0, 2)) + "\\";
                }
                if (Common.machine_is_windows()) {
                    can_path = "/" + can_path;
                }
                String s = "path=" + can_path + ":::name=" + name + ":::size=" + files[x].length() + ":::type=" + (files[x].isDirectory() ? "dir" : "file") + ":::modified=" + files[x].lastModified() + ";;;";
    ...
    public void doUpload(Properties p) {
        ...
        if (this.uploaders.get(p.getProperty("UNIQUE_KEY", "0")) == null) {
            this.uploaders.put(p.getProperty("UNIQUE_KEY", "0"), new Uploader2(new Properties(), this.CrushAuth));
        }
        Uploader2 uploader2 = uploader = (Uploader2)this.uploaders.get(p.getProperty("UNIQUE_KEY", "0"));
        ...
        new Thread(new Runnable(){
            ...
            @Override
            public void run() {
                if (!Applet2.this.ramConfigured) {
                    Applet2.this.ramConfigured = true;
                    Applet2.this.updateRam();
                    Tunnel2.setMaxRam((int)(Applet2.this.availableRam / 0x100000L) - 16);
                }
                Uploader2 uploader2 = uploader;
                synchronized (uploader2) {
                    uploader.go();
                }
            }
    ...
    public void doDownload(Properties p) {
        ...
        if (this.downloaders.get(p.getProperty("UNIQUE_KEY", "0")) == null) {
            this.downloaders.put(p.getProperty("UNIQUE_KEY", "0"), new Downloader2(this.CrushAuth));
        }
        Downloader2 downloader2 = downloader = (Downloader2)this.downloaders.get(p.getProperty("UNIQUE_KEY", "0"));
        ...
        new Thread(new Runnable(){
            ...
            public void run() {
                ...
                Downloader2 downloader2 = downloader;
                synchronized (downloader2) {
                    downloader.go();
                }
            }
    ...
    public void doAction(Properties p) {
        if (p.getProperty("TYPE", "UPLOAD").equals("UPLOAD")) {
            Uploader2 uploader = (Uploader2)this.uploaders.get(p.getProperty("UNIQUE_KEY", "0"));
            ...
            String result_data = "status=" + uploader.getStatus() + ":::totalBytes=" + uploader.getTotalBytes() + ":::transferedBytes=" + uploader.getTransferedBytes() + ":::totalItems=" + uploader.getTotalItems() + ":::transferedItems=" + uploader.getTransferedItems() + uploader.getAsk();
            p.put("result_data", result_data);
        } else {
            Downloader2 downloader = (Downloader2)this.downloaders.get(p.getProperty("UNIQUE_KEY", "0"));
            if (p.getProperty("ACTION", "").equals("PAUSE")) {
                downloader.pause();
            }
            if (p.getProperty("ACTION", "").equals("RESUME")) {
            ...
            String result_data = "status=" + downloader.getStatus() + ":::totalBytes=" + downloader.getTotalBytes() + ":::transferedBytes=" + downloader.getTransferedBytes() + ":::totalItems=" + downloader.getTotalItems() + ":::transferedItems=" + downloader.getTransferedItems() + downloader.getAsk();
            p.put("result_data", result_data);
        }
        p.put("result", "ok");
    }
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    public void doLog(Properties p) {
        StringBuffer sb = new StringBuffer();
        Object object = log_lock;
        synchronized (object) {
            int x = 0;
            while (x < 1000 & log.size() > 0) {
                sb.append(log.remove(0)).append("\r\n");
                ++x;
            }
        }
        p.put("result_data", sb.toString());
        p.put("result", "ok");
    }

    public void doAskResponse(Properties p) {
        if (p.getProperty("TYPE", "UPLOAD").equals("UPLOAD")) {
            Uploader2 uploader = (Uploader2)this.uploaders.get(p.getProperty("UNIQUE_KEY", "0"));
            Properties stat = (Properties)uploader.ask_response.get(p.getProperty("UID"));
            if (stat != null) {
                stat.put("response", p.getProperty("RESPONSE"));
            }
            uploader.ask_response.remove(p.getProperty("UID"));
        } else {
            Downloader2 downloader = (Downloader2)this.downloaders.get(p.getProperty("UNIQUE_KEY", "0"));
            Properties stat = (Properties)downloader.ask_response.get(p.getProperty("UID"));
            if (stat != null) {
                stat.put("response", p.getProperty("RESPONSE"));
            }
            downloader.ask_response.remove(p.getProperty("UID"));
        }
        p.put("result", "ok");
    }
