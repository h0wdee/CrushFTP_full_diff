...
public class IdleManager {
    ...
    public void watch(Folder folder) throws MessagingException {
        ...
        if (this.logger.isLoggable(Level.FINEST)) {
            this.logger.log(Level.FINEST, "IdleManager watching {0}", IdleManager.folderName(ifolder));
        }
        int tries = 0;
        while (!ifolder.startIdle(this)) {
            if (this.logger.isLoggable(Level.FINEST)) {
                this.logger.log(Level.FINEST, "IdleManager.watch startIdle failed for {0}", IdleManager.folderName(ifolder));
            }
            ++tries;
        }
        if (this.logger.isLoggable(Level.FINEST)) {
            if (tries > 0) {
                this.logger.log(Level.FINEST, "IdleManager.watch startIdle succeeded for {0} after " + tries + " tries", IdleManager.folderName(ifolder));
            } else {
                this.logger.log(Level.FINEST, "IdleManager.watch startIdle succeeded for {0}", IdleManager.folderName(ifolder));
            }
        }
    ...
    private void select() {
        block16: {
            ...
            while (!this.die) {
                ...
                if (this.die || Thread.currentThread().isInterrupted()) break;
                do {
                    this.processKeys();
    ...
    private void watchAll() {
        ...
        while ((folder = this.toWatch.poll()) != null) {
            if (this.logger.isLoggable(Level.FINEST)) {
                this.logger.log(Level.FINEST, "IdleManager adding {0} to selector", IdleManager.folderName(folder));
            }
            try {
    ...
    private void processKeys() throws IOException {
        ...
        while (it.hasNext()) {
            ...
            if (this.logger.isLoggable(Level.FINEST)) {
                this.logger.log(Level.FINEST, "IdleManager selected folder: {0}", IdleManager.folderName(folder));
            }
            ...
            try {
                if (folder.handleIdle(false)) {
                    if (this.logger.isLoggable(Level.FINEST)) {
                        this.logger.log(Level.FINEST, "IdleManager continue watching folder {0}", IdleManager.folderName(folder));
                    }
                    this.toWatch.add(folder);
                    continue;
                }
                if (!this.logger.isLoggable(Level.FINEST)) continue;
                this.logger.log(Level.FINEST, "IdleManager done watching folder {0}", IdleManager.folderName(folder));
            }
        ...
        while ((folder = this.toAbort.poll()) != null) {
            SocketChannel sc;
            if (this.logger.isLoggable(Level.FINEST)) {
                this.logger.log(Level.FINEST, "IdleManager aborting IDLE for folder: {0}", IdleManager.folderName(folder));
            }
    ...
    private void unwatchAll() {
        ...
        for (SelectionKey sk : keys) {
            ...
            folder = (IMAPFolder)sk.attachment();
            if (this.logger.isLoggable(Level.FINEST)) {
                this.logger.log(Level.FINEST, "IdleManager no longer watching folder: {0}", IdleManager.folderName(folder));
            }
            SelectableChannel sc = sk.channel();
            try {
                sc.configureBlocking(true);
        ...
        while ((folder = this.toWatch.poll()) != null) {
            SocketChannel sc;
            if (this.logger.isLoggable(Level.FINEST)) {
                this.logger.log(Level.FINEST, "IdleManager aborting IDLE for unwatched folder: {0}", IdleManager.folderName(folder));
            }
            if ((sc = folder.getChannel()) == null) continue;
            try {
                sc.configureBlocking(true);
