...
public class TreeConnect {
    ...
    void close() throws TransportException {
        try {
            ...
            SMB2Packet smb2Packet = (SMB2Packet)Futures.get(send, this.connection.getConfig().getTransactTimeout(), TimeUnit.MILLISECONDS, TransportException.Wrapper);
            if (!NtStatus.isSuccess(((SMB2Header)smb2Packet.getHeader()).getStatusCode())) {
                throw new SMBApiException((SMB2Header)smb2Packet.getHeader(), "Error closing connection to " + this.smbPath);
            }
        }
        finally {
