...
public class InfoCmp {
        ...
        for (int i = 2; i < lines.length; ++i) {
            ...
            while (m.find()) {
                ...
                if (cap.contains("#")) {
                    ...
                    int iVal = val.startsWith("0x") ? Integer.parseInt(val.substring(2), 16) : Integer.parseInt(val);
                    String[] stringArray = InfoCmp.getNames(key);
                ...
                if (cap.contains("=")) {
                    ...
                    String key = cap.substring(0, index);
                    String val = cap.substring(index + 1);
                    for (String name : InfoCmp.getNames(key)) {
                        strings.put(name, val);
                    }
                    continue;
                }
                for (String name : InfoCmp.getNames(cap)) {
                    bools.add(name);
                }
            }
        }
    ...
    static {
        ...
        String[][] stringArrayArray = list;
        int n = stringArrayArray.length;
        for (int i = 0; i < n; ++i) {
            String[] names;
            for (String name : names = stringArrayArray[i]) {
                map.put(name, names);
            }
        }
