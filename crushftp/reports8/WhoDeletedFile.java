...
public class WhoDeletedFile {
    ...
    public void generate(Properties stats, Properties params, StringBuffer sb, Properties status) {
        try {
            ...
            String tmp = Common.getFileText(String.valueOf(System.getProperty("crushftp.web")) + "WebInterface/Reports/WhoDeletedFile.sql");
            if (tmp != null) {
                sql = tmp;
            } else {
                Common.setFileText(sql, String.valueOf(System.getProperty("crushftp.web")) + "WebInterface/Reports/WhoDeletedFile.sql");
            }
            ...
            try (DVector users = ServerStatus.thisObj.statTools.executeSqlQuery(sql, new Object[]{mmddyyyy.parse(params.getProperty("startDate", "1/1/2000 00:00:00")), mmddyyyy.parse(params.getProperty("endDate", "1/1/2100 00:00:00"))}, false, params);){
                ...
                sb.append(Common.getXMLString(results, "results", "WebInterface/Reports/WhoDeletedFile.xsl"));
            }
        }
        catch (Exception e) {
            Log.log("REPORT", 1, e);
