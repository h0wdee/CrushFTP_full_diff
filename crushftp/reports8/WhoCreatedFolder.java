...
public class WhoCreatedFolder {
    ...
    public void generate(Properties stats, Properties params, StringBuffer sb, Properties status) {
        try {
            ...
            String tmp = Common.getFileText(String.valueOf(System.getProperty("crushftp.web")) + "WebInterface/Reports/WhoCreatedFolder.sql");
            if (tmp != null) {
                sql = tmp;
            } else {
                Common.setFileText(sql, String.valueOf(System.getProperty("crushftp.web")) + "WebInterface/Reports/WhoCreatedFolder.sql");
            }
            ...
            try (DVector users = ServerStatus.thisObj.statTools.executeSqlQuery(sql, new Object[]{mmddyyyy.parse(params.getProperty("startDate", "1/1/2000 00:00:00")), mmddyyyy.parse(params.getProperty("endDate", "1/1/2100 00:00:00"))}, false, params);){
                ...
                if ((method = params.getProperty("searchPath", "starts with")).equals("contains")) {
                    path = "%" + path + "%";
                }
                if (method.equals("starts with")) {
                    path = path + "%";
                ...
                DVector allFiles = ServerStatus.thisObj.statTools.executeSqlQuery(sql_files, new Object[]{mmddyyyy.parse(params.getProperty("startDate", "1/1/2000 00:00:00")), mmddyyyy.parse(params.getProperty("endDate", "1/1/2100 00:00:00")), path, foldername}, false, params);
                ...
                if (users_with_activity.size() > 0) {
                    status.put("report_empty", "false");
                }
                sb.append(Common.getXMLString(results, "results", "WebInterface/Reports/WhoCreatedFolder.xsl"));
            }
        }
        catch (Exception e) {
            Log.log("REPORT", 1, e);
