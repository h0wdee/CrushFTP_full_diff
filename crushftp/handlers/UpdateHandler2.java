...
public class UpdateHandler2 {
    ...
    public boolean doSilentUpdate(boolean earlyAccess, String thisVersion, boolean webOnly) throws Exception {
        ...
        String fname = String.valueOf(System.getProperty("appname", "CrushFTP")) + Common.V() + "_" + (Common.machine_is_x() && Common.OSXApp() ? "OSX" : "PC");
        ...
        if (!new File_S(String.valueOf(home) + fname + "_new.zip").exists()) {
            ...
            Worker.startWorker(new Runnable(){
                ...
                public void run() {
                    int x = 0;
                    while (x < files.size()) {
                        UpdateHandler2.this.updateCurrentStatus = "Building list of file differences...:" + (x + 1) + " of " + (files.size() + 1);
                        if (UpdateHandler2.this.stopNow) break;
                        Properties p = (Properties)files.elementAt(x);
        ...
        try {
            ...
            UpdateHandler2.doUpdate(home, backup2, String.valueOf(fname) + "_new.zip");
        ...
        Worker.startWorker(new Runnable(){
            ...
            public void run() {
                ...
                UpdateHandler2.this.updateCurrentStatus = "";
            }
        });
        return true;
    }
    ...
    public static void doUpdate(String home, String backup, String zip) throws Exception {
        ...
        if (UpdateHandler2.unzip(String.valueOf(home) + zip, errors, backup)) {
            String batch = "";
            String manualFiles = "";
    ...
    public String getFileHTTP(String localPath, String name, String ext, String url, String paths, String username, String password) throws Exception {
        Properties config = new Properties();
        config.put("user_dmz", "(current_server)");
        URLConnection urlc = URLConnection.openConnection(new VRL("https://www." + System.getProperty("appname", "CrushFTP").toLowerCase() + ".com/"), config);
        String auth = "Basic " + Base64.encodeBytes((String.valueOf(username) + ":" + password).getBytes());
        urlc.setRequestProperty("Authorization", auth);
        urlc.setRequestMethod("GET");
