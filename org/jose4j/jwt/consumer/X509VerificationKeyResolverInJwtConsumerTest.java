...
public class X509VerificationKeyResolverInJwtConsumerTest {
    ...
    public static void initKeyList() throws JoseException {
        ...
        CERT_LIST = new ArrayList<X509Certificate>();
        String[] stringArray = new String[]{j0, j1, j2, j3, j4};
    ...
    public void x5tStuff() throws Exception {
        ...
        JwtConsumer jwtConsumer = new JwtConsumerBuilder().setVerificationKeyResolver(new X509VerificationKeyResolver(CERT_LIST)).setEvaluationTime(NumericDate.fromSeconds(1420296253L)).setExpectedAudience("you").build();
        JwtClaims jwtClaims = jwtConsumer.processToClaims(jwt);
        Assert.assertThat((Object)"about", (Matcher)CoreMatchers.equalTo((Object)jwtClaims.getSubject()));
        jwtConsumer.processContext(jwtContext);
        Assert.assertThat((Object)"about", (Matcher)CoreMatchers.equalTo((Object)jwtContext.getJwtClaims().getSubject()));
        jwtConsumer = new JwtConsumerBuilder().setVerificationKeyResolver(new X509VerificationKeyResolver(CERT_LIST.get(0), CERT_LIST.get(2), CERT_LIST.get(3), CERT_LIST.get(4))).setEvaluationTime(NumericDate.fromSeconds(1420296253L)).setExpectedAudience("you").build();
        SimpleJwtConsumerTestHelp.expectProcessingFailure(jwt, jwtContext, jwtConsumer);
    }

    @Test
    public void x5tS256Stuff() throws Exception {
        String jwt = "eyJ4NXQjUzI1NiI6IkZTcU90QjV2UHFaNGtqWXAwOUZqQnBrbVhIMFZxRURtLXdFY1Rjb3g2RUUiLCJhbGciOiJFUzI1NiJ9.eyJpc3MiOiJtZSIsImF1ZCI6InlvdSIsImV4cCI6MTQyMDI5OTUzOSwic3ViIjoiYWJvdXQifQ.9Nj3UG8N9u7Eyu0wupR-eVS4Mf0ItwwHBZzwLcY2KUCJeWoPRPT7zC4MqMbHfLj6PzFi09iC3q3PniSJwmWJTA";
        JwtConsumer firstPassConsumer = new JwtConsumerBuilder().setSkipAllValidators().setDisableRequireSignature().setSkipSignatureVerification().build();
        JwtContext jwtContext = firstPassConsumer.process(jwt);
        JwtConsumer jwtConsumer = new JwtConsumerBuilder().setVerificationKeyResolver(new X509VerificationKeyResolver(CERT_LIST)).setEvaluationTime(NumericDate.fromSeconds(1420299538L)).setExpectedAudience("you").build();
        JwtClaims jwtClaims = jwtConsumer.processToClaims(jwt);
        Assert.assertThat((Object)"about", (Matcher)CoreMatchers.equalTo((Object)jwtClaims.getSubject()));
        jwtConsumer.processContext(jwtContext);
        Assert.assertThat((Object)"about", (Matcher)CoreMatchers.equalTo((Object)jwtContext.getJwtClaims().getSubject()));
        jwtConsumer = new JwtConsumerBuilder().setVerificationKeyResolver(new X509VerificationKeyResolver(CERT_LIST.get(0), CERT_LIST.get(1), CERT_LIST.get(2), CERT_LIST.get(3))).setEvaluationTime(NumericDate.fromSeconds(1420299538L)).setExpectedAudience("you").build();
        SimpleJwtConsumerTestHelp.expectProcessingFailure(jwt, jwtContext, jwtConsumer);
    }

    @Test
    public void bothX5headersStuff() throws Exception {
        String jwt = "eyJ4NXQjUzI1NiI6InFTX2JYTlNfSklYQ3JuUmdha2I2b3RFS3Utd0xlb3R6N0tBWjN4UVVPcUUiLCJ4NXQiOiJpSFFLdVNHZVdVR1laQ2c0X1JHSlNJQzBORFEiLCJhbGciOiJFUzI1NiJ9.eyJpc3MiOiJtZSIsImF1ZCI6InlvdSIsImV4cCI6MTQyMDI5OTc2MSwic3ViIjoiYWJvdXQifQ.04qPYooLJN2G0q0LYVepaydszTuhY7jKjqi5IGkNBAWZ-IBlW_pWzkurR1MkO48SbJQK2swmy7Ogfihi1ClAlA";
        JwtConsumer firstPassConsumer = new JwtConsumerBuilder().setSkipAllValidators().setDisableRequireSignature().setSkipSignatureVerification().build();
        JwtContext jwtContext = firstPassConsumer.process(jwt);
        JwtConsumer jwtConsumer = new JwtConsumerBuilder().setVerificationKeyResolver(new X509VerificationKeyResolver(CERT_LIST)).setEvaluationTime(NumericDate.fromSeconds(1420299760L)).setExpectedAudience("you").build();
        JwtClaims jwtClaims = jwtConsumer.processToClaims(jwt);
        Assert.assertThat((Object)"about", (Matcher)CoreMatchers.equalTo((Object)jwtClaims.getSubject()));
        jwtConsumer.processContext(jwtContext);
        Assert.assertThat((Object)"about", (Matcher)CoreMatchers.equalTo((Object)jwtContext.getJwtClaims().getSubject()));
        jwtConsumer = new JwtConsumerBuilder().setVerificationKeyResolver(new X509VerificationKeyResolver(CERT_LIST.get(0), CERT_LIST.get(1), CERT_LIST.get(2), CERT_LIST.get(4))).setEvaluationTime(NumericDate.fromSeconds(1420299760L)).setExpectedAudience("you").build();
        SimpleJwtConsumerTestHelp.expectProcessingFailure(jwt, jwtContext, jwtConsumer);
    }
    ...
    public void noThumbHeader() throws InvalidJwtException, MalformedClaimException {
        ...
        JwtConsumer firstPassConsumer = new JwtConsumerBuilder().setSkipAllValidators().setDisableRequireSignature().setSkipSignatureVerification().build();
        JwtContext jwtContext = firstPassConsumer.process(jwt);
        JwtConsumer jwtConsumer = new JwtConsumerBuilder().setVerificationKeyResolver(new X509VerificationKeyResolver(CERT_LIST.get(0), CERT_LIST.get(1))).setEvaluationTime(NumericDate.fromSeconds(1420298972L)).setExpectedAudience("you").build();
        SimpleJwtConsumerTestHelp.expectProcessingFailure(jwt, jwtContext, jwtConsumer);
        ArrayList<X509Certificate> certificates = new ArrayList<X509Certificate>(CERT_LIST);
        Collections.reverse(certificates);
        ArrayList<List<X509Certificate>> certificateListList = new ArrayList<List<X509Certificate>>();
        ...
        for (List list : certificateListList) {
            X509VerificationKeyResolver verificationKeyResolver = new X509VerificationKeyResolver(list);
            verificationKeyResolver.setTryAllOnNoThumbHeader(true);
            jwtConsumer = new JwtConsumerBuilder().setVerificationKeyResolver(verificationKeyResolver).setEvaluationTime(NumericDate.fromSeconds(1420298972L)).setExpectedAudience("you").build();
            JwtClaims jwtClaims = jwtConsumer.processToClaims(jwt);
            Assert.assertThat((Object)"about", (Matcher)CoreMatchers.equalTo((Object)jwtClaims.getSubject()));
            jwtConsumer.processContext(jwtContext);
            Assert.assertThat((Object)"about", (Matcher)CoreMatchers.equalTo((Object)jwtContext.getJwtClaims().getSubject()));
        }
        X509VerificationKeyResolver x509VerificationKeyResolver = new X509VerificationKeyResolver(CERT_LIST.get(0), CERT_LIST.get(2));
        x509VerificationKeyResolver.setTryAllOnNoThumbHeader(true);
        jwtConsumer = new JwtConsumerBuilder().setVerificationKeyResolver(x509VerificationKeyResolver).setEvaluationTime(NumericDate.fromSeconds(1420298972L)).setExpectedAudience("you").build();
        SimpleJwtConsumerTestHelp.expectProcessingFailure(jwt, jwtContext, jwtConsumer);
    }

    @Test
    public void compareToOpenSslFingerprints() throws Exception {
        this.compareToOpenSslFingerprint("67:30:12:77:0B:B0:31:86:C4:19:57:79:04:FA:A2:79:FF:75:23:4B", "08:9C:BE:94:01:37:5F:46:AB:E3:87:0A:AF:12:9C:6A:E5:07:02:90:FF:92:D1:63:3C:2F:6C:E8:77:8E:C7:35", CERT_LIST.get(0));
        this.compareToOpenSslFingerprint("65:BD:47:4F:BA:F2:09:10:00:8F:6C:1C:43:C8:56:6F:36:AA:5F:5B", "15:20:16:EE:C7:99:83:CB:A2:42:BF:75:0D:CE:D6:18:23:95:E3:A0:BE:5C:E6:4E:C9:EF:1A:E6:07:51:36:46", CERT_LIST.get(1));
        this.compareToOpenSslFingerprint("0E:49:9C:40:CE:40:CC:3F:27:9D:03:95:E4:51:3F:63:5A:34:97:96", "6C:34:95:6E:9E:3D:D2:ED:EE:D7:DE:31:0A:15:FA:CD:76:FF:76:80:6F:9B:5F:CE:D4:F2:4C:F5:FC:5C:3A:20", CERT_LIST.get(2));
        this.compareToOpenSslFingerprint("88:74:0A:B9:21:9E:59:41:98:64:28:38:FD:11:89:48:80:B4:34:34", "A9:2F:DB:5C:D4:BF:24:85:C2:AE:74:60:6A:46:FA:A2:D1:0A:BB:EC:0B:7A:8B:73:EC:A0:19:DF:14:14:3A:A1", CERT_LIST.get(3));
        this.compareToOpenSslFingerprint("5B:C6:8E:F8:10:F6:8F:1F:4A:33:34:23:85:9F:39:BA:42:40:E5:98", "15:2A:8E:B4:1E:6F:3E:A6:78:92:36:29:D3:D1:63:06:99:26:5C:7D:15:A8:40:E6:FB:01:1C:4D:CA:31:E8:41", CERT_LIST.get(4));
    }
    ...
    void compareToOpenSslFingerprint(String sha1fp, String sha256fp, X509Certificate certificate) {
        ...
        Assert.assertArrayEquals((byte[])Hex.decode((String)sha1fp), (byte[])Base64Url.decode(x5t));
        sha256fp = sha256fp.replace(":", "");
        String x5tS2 = X509Util.x5tS256(certificate);
        Assert.assertArrayEquals((byte[])Hex.decode((String)sha256fp), (byte[])Base64Url.decode(x5tS2));
    }
}
