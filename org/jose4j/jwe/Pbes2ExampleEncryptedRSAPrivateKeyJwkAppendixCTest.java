...
public class Pbes2ExampleEncryptedRSAPrivateKeyJwkAppendixCTest {
    ...
    public void encryptExample() throws Exception {
        ...
        Pbes2HmacShaWithAesKeyWrapAlgorithm.HmacSha256Aes128 pbesAlg = new Pbes2HmacShaWithAesKeyWrapAlgorithm.HmacSha256Aes128();
        ...
        Assert.assertArrayEquals((byte[])expectedDerived, (byte[])derivedKey.getEncoded());
        byte[] exampleCek = ByteUtil.convertUnsignedToSignedTwosComp(new int[]{111, 27, 25, 52, 66, 29, 20, 78, 92, 176, 56, 240, 65, 208, 82, 112, 161, 131, 36, 55, 202, 236, 185, 172, 129, 23, 153, 194, 195, 48, 253, 182});
        AesKeyWrapManagementAlgorithm.Aes128 keyWrap = new AesKeyWrapManagementAlgorithm.Aes128();
        ContentEncryptionKeyDescriptor cekDesc = new ContentEncryptionKeyDescriptor(exampleCek.length, "AES");
        ...
        Assert.assertArrayEquals((byte[])exampleCek, (byte[])contentEncryptionKey);
        byte[] expectedEncryptedKey = ByteUtil.convertUnsignedToSignedTwosComp(new int[]{78, 186, 151, 59, 11, 141, 81, 240, 213, 245, 83, 211, 53, 188, 134, 188, 66, 125, 36, 200, 222, 124, 5, 103, 249, 52, 117, 184, 140, 81, 246, 158, 161, 177, 20, 33, 245, 57, 59, 4});
        byte[] encryptedKey = contentEncryptionKeys.getEncryptedKey();
        String encodedEncryptedKey = base64url.base64UrlEncode(encryptedKey);
        Assert.assertArrayEquals((byte[])expectedEncryptedKey, (byte[])encryptedKey);
        ...
        Assert.assertThat((Object)CS, (Matcher)CoreMatchers.is((Matcher)CoreMatchers.equalTo((Object)cs)));
    }
}
